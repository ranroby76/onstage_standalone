...
void ManualTab::buttonClicked(juce::Button* b) {
    ...
    else if (b == &btnDemoLimitations) showChapter(DemoLimitations);  // FIX: Added new handler
}
...
void ManualTab::highlightButton(Chapter chapter) {
    ...
    switch (chapter) {
        ...
        case DemoLimitations: activeBtn = &btnDemoLimitations; break;  // FIX: Added new case
    }
    ...
    switch (chapter) {
        case Introduction:
            return juce::String(
                ...
                "* Left Panel - Main tab selector (Rack, Mixer, Studio, etc.)\n"
                "* Main Tabs - Rack, Mixer, Studio, Settings, Plugins, Manual, Register\n"
                "* Bottom Footer - Instrument Selector (32 slots, 2 rows)\n"
        ...
        case RackView:
            return juce::String(
                ...
                "---------------------------------------------------------------\n"
                "* 32 instrument slots arranged in 2 rows of 16\n"
                "* Click any slot to activate/deactivate an instrument\n"
                "* Multi-Mode toggle: Switch between single and multi-instrument mode\n"
                "  - Single Mode: Only one instrument active at a time\n"
                "  - Multi-Mode: Multiple instruments can be active simultaneously\n\n"
                "MIDI NOTE CONTROL\n"
                "---------------------------------------------------------------\n"
                "* MIDI notes 1-32 trigger instrument selection\n"
                "* Note 1 = Instrument 1, Note 2 = Instrument 2, etc.\n"
                "* Right-click any slot for MIDI controller info\n"
                "* Perfect for MIDI pad controllers and switchers\n"
                "* Note: MIDI notes 1-32 are reserved for selection only\n"
            );
        ...
        case Mixer:
            return juce::String(
                ...
                "                           MIXER\n"
                "===============================================================\n\n"
                "The Mixer provides gain control for audio I/O channels and instruments.\n\n"
                "MIXER ROWS\n"
                "---------------------------------------------------------------\n"
                "The mixer is divided into three horizontal rows:\n"
                "* INPUTS - Audio interface input channels\n"
                "* OUTPUTS - Audio interface output channels\n"
                "* INSTRUMENTS - Loaded VST instruments\n\n"
                "CHANNEL STRIPS\n"
                "---------------------------------------------------------------\n"
                "Each channel strip displays:\n"
                "* Channel name (from audio interface or plugin name)\n"
                "* Vertical gain slider (-60dB to +25dB)\n"
                "* Current gain value in dB at top of slider\n"
                "* Unity gain (0dB) is at center position\n\n"
                "INPUT CHANNELS\n"
                "---------------------------------------------------------------\n"
                ...
                "INSTRUMENT CHANNELS\n"
                "---------------------------------------------------------------\n"
                "* Shows all loaded instruments from the Rack\n"
                "* Adjust individual instrument output levels\n"
                "* Useful for balancing multiple instruments\n"
                "* Updates automatically when instruments are added/removed\n\n"
                "GAIN CONTROL\n"
                "---------------------------------------------------------------\n"
                "* Drag slider up/down to adjust gain\n"
                "* Range: -60dB (near silence) to +25dB (boost)\n"
                "* 0dB = Unity gain (no change)\n"
                "* Gain is applied in real-time during playback\n"
            );
        ...
        case Troubleshooting:
            return juce::String(
                ...
                "3. Go to Audio/MIDI tab, verify device is selected\n"
                "4. Check that Audio Output node is visible in Rack\n"
