...
// Shown when user clicks "T" button on a plugin node or container node
...
public:
        ...
        : meteringProc(proc)
    {
        ...
        if (meteringProc)
        {
            ...
            selectTimeSig(meteringProc->getCustomTimeSigNumerator(),
                          meteringProc->getCustomTimeSigDenominator());
...
private:
    ...
    void selectTimeSig(int num, int den)
    {
        ...
        for (auto& s : sigs)
            ...
            { timeSigCombo.setSelectedId(s.id, juce::dontSendNotification); return; }
        timeSigCombo.setSelectedId(3, juce::dontSendNotification);
    }
...
// =============================================================================
// ContainerTransportOverrideComponent
// Identical UI to TransportOverrideComponent but targets ContainerProcessor
// =============================================================================
class ContainerTransportOverrideComponent : public juce::Component,
                                             private juce::Slider::Listener,
                                             private juce::ComboBox::Listener,
                                             private juce::Button::Listener
{
public:
    ContainerTransportOverrideComponent(ContainerProcessor* proc)
        : containerProc(proc)
    {
        setSize(260, 160);

        titleLabel.setText("Container's Transport", juce::dontSendNotification);
        titleLabel.setFont(juce::Font(juce::FontOptions(14.0f, juce::Font::bold)));
        titleLabel.setColour(juce::Label::textColourId, juce::Colours::white);
        titleLabel.setJustificationType(juce::Justification::centred);
        addAndMakeVisible(titleLabel);
        ...
        yesButton.setButtonText("Yes");
        yesButton.setRadioGroupId(1002);
        yesButton.setClickingTogglesState(true);
        yesButton.addListener(this);
        yesButton.setColour(juce::TextButton::buttonOnColourId, juce::Colours::green.darker());
        yesButton.setColour(juce::TextButton::buttonColourId, juce::Colour(0xff3a3a3a));
        ...
        if (containerProc)
        {
            ...
            yesButton.setToggleState(synced, juce::dontSendNotification);
            noButton.setToggleState(!synced, juce::dontSendNotification);
            tempoSlider.setValue(containerProc->getCustomTempo(), juce::dontSendNotification);
            selectTimeSig(containerProc->getCustomTimeSigNumerator(),
                          containerProc->getCustomTimeSigDenominator());
            updateEnabledState(synced);
        }
    }
...
private:
    void buttonClicked(juce::Button* button) override
    {
        ...
        {
            bool synced = yesButton.getToggleState();
            containerProc->setTransportSynced(synced);
            updateEnabledState(synced);
        }
    }
    ...
    void comboBoxChanged(juce::ComboBox* combo) override
    {
        if (!containerProc || combo != &timeSigCombo) return;
        int num = 4, den = 4;
        parseTimeSig(timeSigCombo.getSelectedId(), num, den);
        containerProc->setCustomTimeSignature(num, den);
    }

    void sliderValueChanged(juce::Slider* slider) override
    {
        if (!containerProc || slider != &tempoSlider) return;
        containerProc->setCustomTempo(tempoSlider.getValue());
    }
    ...
    ContainerProcessor* containerProc = nullptr;
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(ContainerTransportOverrideComponent)
};
