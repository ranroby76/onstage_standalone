...
// Simple Pitch Shifter using overlap-add time stretching
...
public:
    void prepare(double sampleRate, int maxBlockSize)
    {
        ...
        inputBuffer.resize(windowSize * 2, 0.0f);
        outputBuffer.resize(windowSize * 2, 0.0f);
    ...
    void processSample(float input, float& output)
    {
        ...
        int idx1 = readInt % windowSize;
        int idx2 = (readInt + 1) % windowSize;
        output = inputBuffer[idx1] * (1.0f - frac) + inputBuffer[idx2] * frac;
...
public:
    ...
    void process(Context&& ctx)
    {
        ...
        float targetShift[2];
        for (int v = 0; v < 2; ++v)
            targetShift[v] = params.voices[v].enabled ? params.voices[v].fixedSemitones + params.voices[v].centsDetune * 0.01f : 0.0f;
