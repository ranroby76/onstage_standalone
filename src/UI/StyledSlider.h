...
public:
    ...
    {
        // FIX: Removed 'const' so we can modify activeArea with removeFromRight
        auto activeArea = button.getActiveArea();
        const auto isFrontTab = button.isFrontTab();
        
        // Active = Dark Grey, Inactive = Gold
        ...
        // Add Separator Line for Inactive Tabs
        if (!isFrontTab)
        {
            g.setColour(juce::Colour(0xFF202020)); // Dark Grey Separator
            // Draw 1px line on the right edge. This shrinks activeArea by 1px.
...
public:
    ...
    void mouseUp(const juce::MouseEvent& e) override { if (e.mods.isRightButtonDown()) return; juce::Slider::mouseUp(e); }
    void mouseDrag(const juce::MouseEvent& e) override { if (e.mods.isRightButtonDown()) return; juce::Slider::mouseDrag(e); }
private:
...
public:
    ...
    void mouseDown(const juce::MouseEvent& e) override { if (e.mods.isRightButtonDown()) { if (midiInfo.isNotEmpty()) showMidiTooltip(this, midiInfo); return; } Component::mouseDown(e); }
private:
    StyledSlider slider;
    class InternalLabel : public juce::Label {
    public: void setMidiInfo(const juce::String& info) { midiInfo = info; }
        void mouseDown(const juce::MouseEvent& e) override { if (e.mods.isRightButtonDown()) { if (midiInfo.isNotEmpty()) showMidiTooltip(this, midiInfo); return; } juce::Label::mouseDown(e); }
        juce::String midiInfo;
    };
    InternalLabel label;
    juce::String midiInfo;
...
class MidiTooltipToggleButton : public juce::ToggleButton {
public: MidiTooltipToggleButton(const juce::String& text = "") : juce::ToggleButton(text) {}
    ...
    void mouseUp(const juce::MouseEvent& e) override { if (e.mods.isRightButtonDown()) return; juce::ToggleButton::mouseUp(e); }
private: juce::String midiInfo;
};

class MidiTooltipTextButton : public juce::TextButton {
public: MidiTooltipTextButton(const juce::String& text = "") : juce::TextButton(text) {}
    ...
    void mouseUp(const juce::MouseEvent& e) override { if (e.mods.isRightButtonDown()) return; juce::TextButton::mouseUp(e); }
private: juce::String midiInfo;
};

class MidiTooltipLabel : public juce::Label {
