      ...
      mediaPage(p)
{
    ...
    juce::File fananLogoFile = assetsDir.getChildFile("fanan_logo.png");
    if (!fananLogoFile.existsAsFile()) {
        fananLogoFile = deployDir.getChildFile("fanan_logo.png");
    }
    if (!fananLogoFile.existsAsFile()) {
        fananLogoFile = juce::File("D:/Workspace/onstage_colosseum_upgrade/assets/fanan_logo.png");
    }
    
    juce::File onstageLogoFile = assetsDir.getChildFile("onstage_logo.png");
    if (!onstageLogoFile.existsAsFile()) {
        onstageLogoFile = deployDir.getChildFile("onstage_logo.png");
    }
    if (!onstageLogoFile.existsAsFile()) {
        onstageLogoFile = juce::File("D:/Workspace/onstage_colosseum_upgrade/assets/onstage_logo.png");
    }
    ...
    if (onstageLogoFile.existsAsFile()) {
        onstageLogo = juce::ImageFileFormat::loadFrom(onstageLogoFile);
    }
...
void SubterraneumAudioProcessorEditor::paint(juce::Graphics& g) { 
    ...
    if (onstageLogo.isValid()) {
        ...
        float srcAspect = (float)onstageLogo.getWidth() / (float)onstageLogo.getHeight();
        int onstageHeight = logoHeight;
        int onstageWidth = (int)(onstageHeight * srcAspect);
        
        // Center in header area
        int centerArea = getWidth() - Style::leftMenuWidth;
        int onstageX = Style::leftMenuWidth + (centerArea - onstageWidth) / 2;
        int onstageY = logoY + (logoHeight - onstageHeight) / 2;
        
        // Draw with proper source and destination rectangles
        g.drawImage(onstageLogo, 
                    onstageX, onstageY, onstageWidth, onstageHeight,
                    0, 0, onstageLogo.getWidth(), onstageLogo.getHeight(),
